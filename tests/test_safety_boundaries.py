...
@pytest.mark.parametrize(
    "agent,role",
    [
        ("researcher", Role.EDIT),
        ("executor", Role.EDIT),
        ("inspector", Role.REVIEW),
        ("inspector_2", Role.REVIEW),
    ],
)
...
def test_environment_does_not_inherit_secrets():
    env = child_environment(
        {
            "PATH": "tool-path",
            "AWS_SECRET_ACCESS_KEY": "fictional",
            "GITHUB_TOKEN": "fictional",
...
@pytest.mark.parametrize(
    "message", ["insufficient_quota", "billing required", "unauthorized", "policy denied"]
)
...
def test_discovered_project_command_never_runs(tmp_path, monkeypatch):
    ...
    check = congress2.VerificationCheck(
        id="fictional",
        title="test",
        phase="final",
        check_type="command",
...
def test_descendants_are_terminated(tmp_path):
    ...
    watcher.start()
    result = run_process(
        [sys.executable, "-c", parent], cwd=tmp_path, env=child_environment(), timeout=12, cancel=cancel
    )
...
def test_adapter_invalidates_modified_review(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(adapter, "check_version", lambda *args: "0.153.4")
