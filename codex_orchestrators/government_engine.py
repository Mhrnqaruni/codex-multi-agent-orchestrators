"""
Government — phased Executor/Inspector workflow (experimental).
===============================================
...
class TerminalUI:
    ...
    def phase_header(self, phase_num: int, step: str, round_num: int, max_rounds: int):
        ...
        print(f"  {C_CYAN}{C_BOLD}  Phase {phase_num} | {step} | Round {round_num}/{max_rounds}{C_RESET}")
        print(f"  {C_CYAN}{C_BOLD}{'=' * (self._width - 4)}{C_RESET}")
        print()
    ...
    def user_checkpoint(
        self, phase_num: int, executor_sid: str, inspector_sid: str, auto_timeout: int = 0
    ) -> str:
        print()
        ...
        if auto_timeout > 0:
            ...
            for remaining in range(auto_timeout, 0, -1):
                ...
                if remaining % 30 == 0 or remaining <= 10:
                    sys.stdout.write(
                        f"\r  {C_DIM}Auto-continuing in {remaining}s... (press any key to choose){C_RESET}   "
                    )
                    sys.stdout.flush()
                time.sleep(1)
            else:
                # Timeout expired — auto-continue
                sys.stdout.write(f"\r  {C_GREEN}Auto-continuing to next phase...{' ' * 30}{C_RESET}\n")
                sys.stdout.flush()
                return "c"
    ...
    def __init__(self, gov_dir: str):
        ...
        if os.path.exists(self.state_path):
            try:
                ...
                _print_safe(f"  {C_YELLOW}[WARNING] state.json corrupted ({e}), starting fresh.{C_RESET}")
    ...
    if os.path.exists(expected_path):
        try:
            ...
            if len(content.strip()) < min_chars:
                return (
                    False,
                    f"File exists but is too short ({len(content.strip())} chars). Write the actual content.",
                )
            return True, "OK"
        except OSError as e:
    ...
    if os.path.isdir(expected_dir):
        for fname in os.listdir(expected_dir):
            if fname.lower() == expected_name:
                ...
                return False, (
                    f"File created with wrong name: '{fname}'. "
        ...
        for fname in os.listdir(expected_dir):
            if stem in fname.lower() and fname.lower().endswith(".md"):
                return False, (
                    f"Found similar file '{fname}' but expected exact name "
...
class Government:
    ...
    def __init__(
        ...
        if self._source_changed:
            self.ui.status(f"Source file changed: {old_source} -> {self.source_file}", C_YELLOW)
            self.logger.master("SYSTEM", f"Source file changed from {old_source} to {self.source_file}")
        self.state.update(
    ...
    def _check_pause(self):
        ...
        try:
            while True:
                ...
                if wait_count % 15 == 0:
                    self.ui.status(
                        f"Still paused ({wait_count * 2}s)... delete .government/pause to continue", C_DIM
                    )
        ...
        try:
            ...
            while True:
                ...
                if wait % 75 == 0:  # every 15s
                    self.ui.status(f"Still paused ({wait // 5}s)... [R=resume, Q=quit]", C_DIM)
        finally:
    ...
    def _wait_for_rate_limit(self, retry_info: str) -> str:
        ...
        self.ui.status(
            f"RATE LIMITED — press [R] to retry now, [Q] to quit. Auto-retry in {auto_minutes}m.{retry_info}",
            C_YELLOW,
        )
        ...
        try:
            while True:
                if _kbhit():
                    ...
                    if ch == "r":
                        self.logger.master("SYSTEM", f"Rate limit manual retry after {waited}s.{retry_info}")
                        return "manual_retry"
                    elif ch == "q":
                        self._interrupt_requested = True
                        self.ui.status("Quit requested.", C_RED)
                        self.logger.master("SYSTEM", f"Rate limit quit after {waited}s.{retry_info}")
                        return "quit"
                now = time.monotonic()
                if now >= deadline:
                    waited = int(now - start)
                    self.logger.master("SYSTEM", f"Rate limit auto-retry after {waited}s.{retry_info}")
                    return "auto_retry"
                time.sleep(1)
                waited = int(time.monotonic() - start)
                ...
                if waited >= 30 and status_bucket != last_status_bucket:
                    ...
                    self.ui.status(
                        f"Rate limited ({waited}s)... auto-retry in {remaining}s [R=retry now, Q=quit]", C_DIM
                    )
        finally:
            _exit_cbreak(old_term)
    ...
    def _run_with_recovery(
        ...
        while True:
            ...
            if rc == -3 or _is_network_error(stderr, rc):
                self.logger.master(agent.upper(), f"NETWORK ERROR (attempt {attempt + 1}): {stderr[:300]}")
                ...
                network_retries += 1
                attempt += 1
                self.ui.status(f"Retrying {agent} after network recovery (attempt {attempt + 1})...", C_CYAN)
                continue
            ...
            if rate_limited:
                ...
                self.logger.master(agent.upper(), f"RATE LIMIT (attempt {attempt + 1}): {stderr[:300]}")
                ...
                if wait_action == "auto_retry":
                    self.ui.status(
                        f"Auto-retrying {agent} after rate-limit wait (attempt {attempt + 1})...", C_CYAN
                    )
                else:
                    self.ui.status(f"Retrying {agent} on user request (attempt {attempt + 1})...", C_CYAN)
                continue
    ...
    def _call_agent(self, agent: str, prompt: str, action_desc: str) -> str:
        ...
        if is_session_error:
            ...
            self.logger.master(
                agent.upper(), f"Resume failed (rc={rc}, stderr={stderr[:200]}), falling back to new session"
            )
            ...
            if not stdout.strip() and rc != 0:
                msg = (
                    f"Fallback session for {agent} also returned no output (rc={rc}). "
        ...
        if agent == "inspector" and output and rc == 0:
            ...
            write_government_review(
                ...
                self.state.get("current_step", "init"),
                self.state.get("current_phase", 0),
                output,
            )
        self.ui.status(
    ...
    def _verify_and_retry(
        self, agent: str, expected_path: str, retry_prompt_template: str, min_chars: int = 50
    ) -> bool:
        ...
        for attempt in range(FILE_VERIFY_RETRIES):
            ...
            if attempt < FILE_VERIFY_RETRIES - 1:
                fix_prompt = retry_prompt_template.format(error_message=msg, filepath=expected_path)
                if agent == "executor":
...
"""
    ...
    def _do_master_plan(self) -> bool:
        ...
        if resuming:
            ...
            self.ui.status(
                f"Resuming master plan at round {saved_round} (substep: {saved_substep or 'start'})", C_GREEN
            )
            self.logger.master(
                "SYSTEM", f"Resuming master plan at round {saved_round}, substep={saved_substep}"
            )
        else:
            round_num = 0
            self.state.update(current_step="master_plan", current_round=0, current_substep="")
        ...
        while True:
            ...
            skip_executor = (
                first_iteration and resuming and round_num == saved_round and saved_substep == "executor_done"
            )
            first_iteration = False

            if skip_executor:
                self.ui.status(
                    f"Executor already completed round {round_num}, skipping to inspector.", C_GREEN
                )
            else:
            ...
            if not self._verify_and_retry(
                "inspector", master_review_path, self.FILE_RETRY_PROMPT, min_chars=30
            ):
                return False
            ...
            if not has_v:
                ...
                fix = (
                    f"You forgot to include a verdict in your review at {master_review_path}. "
            ...
            if round_num >= rounds_limit:
                choice = self.ui.soft_stop_prompt(
                    f"Master plan review: {round_num} rounds without approval", self.max_rounds
                )
                if choice == "c":
                    rounds_limit += self.max_rounds
                    ...
                    self.ui.status(
                        "Skipping to execution with current plan (NOT inspector-approved).", C_YELLOW
                    )
                    self.logger.master(
                        "SYSTEM", f"Master plan SKIPPED by user at round {round_num} (not approved)"
                    )
                    return "skipped"
                else:
                    return False
    ...
    def _do_phase_plan(self, phase_num: int) -> bool:
        ...
        if phase_num > 0:
            ...
            if os.path.exists(prev_report):
                prev_ref = (
                    f"- Previous phase execution report: {prev_report}\n"
        ...
        saved_round = self.state.get("current_round", 0)
        saved_substep = self.state.get("current_substep", "")
        resuming = saved_step == "plan" and saved_round > 0 and self.state.get("current_phase") == phase_num

        if resuming:
            round_num = saved_round - 1
            self.ui.status(
                f"Resuming Phase {phase_num} planning at round {saved_round}"
                f" (substep: {saved_substep or 'start'})",
                C_GREEN,
            )
            self.logger.master(
                "SYSTEM", f"Resuming Phase {phase_num} plan at round {saved_round}, substep={saved_substep}"
            )
        else:
            round_num = 0
            self.state.update(
                current_phase=phase_num, current_step="plan", current_round=0, current_substep=""
            )
            generate_project_status(self.state, self.working_dir)
        ...
        while True:
            ...
            if skip_executor:
                ...
                prompt = build_executor_prompt(self.state, self.working_dir, task)
                self._call_agent("executor", prompt, f"Phase {phase_num} Plan (round {round_num})")
                if self._interrupt_requested:
                    return False
            ...
            self._call_agent("inspector", prompt, f"Review Phase {phase_num} Plan (round {round_num})")
            if self._interrupt_requested:
                return False

            if not self._verify_and_retry("inspector", review_path, self.FILE_RETRY_PROMPT, min_chars=30):
                return False
            ...
            if not has_v:
                fix = f"Add VERDICT: APPROVED or VERDICT: NEEDS_REVISION to the end of {review_path}"
                p = build_inspector_prompt(self.state, self.working_dir, fix)
            ...
            if round_num >= rounds_limit:
                choice = self.ui.soft_stop_prompt(
                    f"Phase {phase_num} plan: {round_num} rounds", self.max_rounds
                )
                if choice == "c":
                    rounds_limit += self.max_rounds
                elif choice == "s":
                    self.ui.status(
                        f"Skipping Phase {phase_num} plan review (NOT inspector-approved).", C_YELLOW
                    )
    ...
    def _do_phase_execution(self, phase_num: int) -> bool:
        ...
        saved_round = self.state.get("current_round", 0)
        saved_substep = self.state.get("current_substep", "")
        resuming = saved_step == "exec" and saved_round > 0 and self.state.get("current_phase") == phase_num
        ...
        if resuming:
            ...
            self.logger.master(
                "SYSTEM", f"Resuming Phase {phase_num} exec at round {saved_round}, substep={saved_substep}"
            )
        else:
            round_num = 0
        ...
        while True:
            ...
            if skip_executor:
                ...
                prompt = build_executor_prompt(self.state, self.working_dir, task)
                self._call_agent("executor", prompt, f"Phase {phase_num} Execute (round {round_num})")
                if self._interrupt_requested:
                    return False
            ...
            prompt = build_inspector_prompt(self.state, self.working_dir, task)
            archive_file(review_path, phase_dir)
            self._call_agent("inspector", prompt, f"Review Phase {phase_num} Execution (round {round_num})")
            if self._interrupt_requested:
                return False
            ...
            if round_num >= rounds_limit:
                choice = self.ui.soft_stop_prompt(
                    f"Phase {phase_num} execution: {round_num} rounds", self.max_rounds
                )
                if choice == "c":
                    rounds_limit += self.max_rounds
                elif choice == "s":
                    self.ui.status(
                        f"Skipping Phase {phase_num} execution review (NOT inspector-approved).", C_YELLOW
                    )
                    self.logger.master(
                        "EXEC", f"Phase {phase_num} execution SKIPPED by user at round {round_num}"
                    )
                    return "skipped"
                else:
                    return False
    ...
    def run(self):
        ...
        if self._resume_mode:
            ...
            can_resume = saved_step not in ("init", "completed") and (
                self.state.get("executor_session_id") or completed_phases
            )
        ...
        if phase_range is None:
            ...
            try:
                start_phase = int(
                    input(f"  {C_CYAN}> First phase number (0 or 1) [1]: {C_RESET}").strip() or "1"
                )
                total_input = int(input(f"  {C_CYAN}> How many phases? {C_RESET}").strip())
        ...
        self.ui.status(f"Phases: {start_phase} to {end_phase} ({total_count} total)", C_CYAN)
        ...
        if start_phase == 0 and 0 not in completed_phases and any(p > 0 for p in completed_phases):
            completed_phases.append(0)
            self.state.set("phases_completed", completed_phases)
            self.ui.status("Phase 0 auto-marked as completed (prerequisite for later phases).", C_GREEN)
        ...
        for phase_num in range(start_phase, end_phase + 1):
            ...
            if skip_plan:
                self.ui.status(
                    f"Phase {phase_num} plan already done (resuming at {saved_step}), skipping.", C_GREEN
                )
            else:
            ...
            if exec_result == "skipped":
                self.ui.status(
                    f"Phase {phase_num} execution proceeding (user-skipped, NOT verified).", C_YELLOW
                )
            else:
...
def main():
    ...
    if not source_file:
        ...
        if not working_dir:
            ...
            try:
                wd_input = (
                    input(f"  {C_CYAN}> Working directory [{default_wd}]: {C_RESET}").strip().strip('"')
                )
            except EOFError:
                print(
                    f"\n  {C_RED}No input (non-interactive). Use: python government.py <directory>{C_RESET}"
                )
                sys.exit(1)
        ...
        if os.path.exists(state_path):
            try:
                ...
                while True:
                    try:
                        choice = (
                            input(f"  {C_CYAN}> Choice [{'/'.join(valid_choices)}]: {C_RESET}")
                            .strip()
                            .lower()
                        )
                    except EOFError:
                ...
                if choice == "q":
                    ...
                    while True:
                        try:
                            new_source = (
                                input(f"  {C_CYAN}> New plan file path: {C_RESET}").strip().strip('"')
                            )
                        except EOFError:
                    ...
                    if old_instr:
                        print(
                            f"  {C_DIM}Previous instructions: {old_instr[:100]}{'...' if len(old_instr) > 100 else ''}{C_RESET}"
                        )
                    try:
                        extra = input(
        ...
        if not source_file:
            ...
            while True:
                try:
                    source_file = input(f"  {C_CYAN}> Source file path: {C_RESET}").strip().strip('"')
                except EOFError:
