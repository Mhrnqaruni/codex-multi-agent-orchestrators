"""
Congress — durable Researcher/Inspector workflow (experimental).
=====================================
...
New in v2:
  ...
  - Cancellation via Ctrl+C and bounded process-tree cleanup
  - Connectivity classified from actual CLI results, without vendor probes
  - Rate limit: bounded retry before resumable stop
...
from .congress_config import (
    ...
    RATE_LIMIT_AUTO_RETRY_SECONDS,
    MAX_CONTINUATIONS,
    INTERNET_CHECK_INTERVAL,
    SOURCE_MANIFEST_MAX_FILES,
    ...
    SOURCE_CONTEXT_EXCLUDED_SUMMARY_MAX,
    ...
    VERIFICATION_MODES,
    DEFAULT_MAX_VERIFICATION_TIMEOUT,
    MIN_MAX_VERIFICATION_TIMEOUT,
    VERIFICATION_OUTPUT_EXCERPT_CHARS,
    DEFAULT_SECOND_INSPECTOR_MODE,
    SECOND_INSPECTOR_MODES,
    DEFAULT_LOG_PROMPTS_MODE,
    LOG_PROMPTS_MODES,
    DEFAULT_INTERACTIVE_CI_MODE,
    ...
    REVIEW_VERDICT_NEEDS_REVISION,
    REVIEW_STATUS_APPROVED,
    REVIEW_STATUS_NEEDS_REVISION,
    REVIEW_STATUS_MALFORMED,
    REVIEW_STATUS_STALE,
    QUALITY_MODE_BEST,
    QUALITY_MODE_STANDARD,
    QUALITY_MODES,
    DEFAULT_QUALITY_MODE,
    QUALITY_MATERIAL_SEVERITIES,
...
from .congress_prompts import RESEARCHER_SYSTEM_PROMPT, INSPECTOR_SYSTEM_PROMPT_TEMPLATE
...
def _stderr_error_lines(stderr: str) -> str:
    ...
    for line in (stderr or "").splitlines():
        ...
        if timestamped_error:
            in_transcript_block = False
        rate_or_usage = any(re.search(pattern, stripped, re.IGNORECASE) for pattern in RATE_LIMIT_PATTERNS)
        context_error = _has_context_limit_signal(stripped)
        network_error = any(sig in lower for sig in NETWORK_ERROR_SIGNALS)
        known_codex_warning = (
            "failed to refresh available models" in lower or "failed to record rollout items" in lower
        )
        if (
            starts_error
...
def _assessment_matches_current_outputs(assessment: dict | None, output_files: list[dict]) -> list[str]:
    ...
    for item in output_files:
        ...
        if str(current_hash) not in referenced_hashes:
            stale.append(
                f"{path} current hash is not referenced by the {assessment.get('reviewer', 'reviewer')} review."
            )
    return stale


def _normalize_result_file_path(
    working_dir: str, raw_path: str | None, output_files: list[str] | None = None
) -> str:
...
def _review_assessments_to_summary(
    ...
    resolved_quality_mode = _normalize_quality_mode(
        quality_mode or (latest_1 or {}).get("quality_mode") or (latest_2 or {}).get("quality_mode"),
        strict=False,
    )
...
def render_congress_result_markdown(result: CongressResult) -> str:
    ...
    if excellence:
        ...
        for item in excellence[:12]:
            lines.append(
                f"- `{item.get('reviewer', 'congress')}` {item.get('label')}: `{item.get('status')}`"
            )
    ...
    if assessments:
        ...
        for item in assessments[-4:]:
            ...
            if quality_item_reasons:
                lines.append(
                    "  - Quality blockers: " + "; ".join(str(reason) for reason in quality_item_reasons[:4])
                )
...
def _render_history_event(result: CongressResult) -> str:
    ...
    if result.output_files:
        for item in result.output_files:
            ...
            if item.get("invalid_type"):
                exists = "invalid"
            lines.append(
                f"  - `{item.get('path')}`: {exists}, sha256: `{item.get('sha256') or 'unavailable'}`"
            )
    else:
        lines.append("  - (none)")
    if result.blocked:
    ...
    if inventory:
        ...
        if capability_hints:
            lines.append(
                "  - Capability hints: `" + ", ".join(str(item) for item in capability_hints[:20]) + "`"
            )
    lines.append("")
    return "\n".join(lines)
...
def _build_lock_metadata(
    working_dir: str, session_id: str, status: str, owner_token: str | None = None
) -> dict:
    now = time.time()
    return {
        "lock_version": 1,
...
def _acquire_workspace_lock_file(
    working_dir: str,
    session_id: str,
            ...
            if stale_reason:
                ...
                try:
                    ...
                    if current_read_error:
                        raise WorkspaceLockError(
                            "Workspace lock became unreadable during stale takeover: " + current_read_error
                        )
...
def _release_workspace_lock_file(working_dir: str, session_id: str, owner_token: str | None) -> bool:
    path = _workspace_lock_path(working_dir)
    if not os.path.exists(path):
        return True
...
def _resume_lock_conflict(working_dir: str, session_id: str | None) -> str | None:
    ...
    if metadata and session_id and metadata.get("session_id") == session_id:
        ...
        return None
    return "Workspace lock is held by another Congress session: " + _format_lock_owner(metadata)
...
def _validate_resume_state(saved: dict | None, working_dir: str) -> dict:
    ...
    if not normalized:
        ...
        if phase == PHASE_UNKNOWN and not (
...
def _write_round_output_status(working_dir: str, round_num: int, before: list[dict], after: list[dict]):
...
def _render_source_manifest_section(source_manifest: list[dict] | None) -> str:
    ...
    for tier in (SOURCE_CONTEXT_TIER_PROJECT, SOURCE_CONTEXT_TIER_OPTIONAL):
        ...
        lines.append(
            "These files are optional context. Inspect them as needed; do not mechanically read unrelated optional files."
        )
    ...
    if excluded_items:
        ...
        lines.append(
            "These paths are excluded by default unless the user directly requests them. Sensitive excluded file names are summarized, not listed."
        )
...
def _render_capability_inventory_section(inventory: dict | None) -> str:
    ...
    if tools:
        tool_bits = [
            f"{name}={'present' if present else 'missing'}" for name, present in sorted(tools.items())
        ]
    ...
    if preflight_results:
        ...
        for item in preflight_results:
            req = item.get("requirement") or {}
            label = (
                req.get("label") or req.get("value") or req.get("name") or req.get("url") or req.get("type")
            )
            state = "pass" if item.get("ok") else "fail"
            lines.append(f"  - `{label}`: {state} - {item.get('message', 'no message')}")
    return "\n".join(lines) + "\n\n"
...
def _render_verification_context_section(
    classification: dict | None, verification_summary: dict | None
) -> str:
...
def _render_review_context_section(review_summary: dict | None, audience: str = "default") -> str:
...
def _normalize_state_v3(data: dict | None, working_dir: str | None = None) -> dict | None:
    ...
    explicit_manual = interactive_requested or (ci_mode_explicit and data.get("ci_mode") is False)
    ...
    state = {
        ...
        "max_verification_timeout": _safe_int(
            data.get("max_verification_timeout"), DEFAULT_MAX_VERIFICATION_TIMEOUT
        ),
        ...
        "inspector_1_status": data.get("inspector_1_status")
        or (data.get("review_state") or {}).get("inspector_1_status"),
        "inspector_2_status": data.get("inspector_2_status")
        or (data.get("review_state") or {}).get("inspector_2_status"),
    ...
    *,
    ...
    base = {
        ...
        "required_source_files": list(
            required_source_files if required_source_files is not None else source_files
        ),
        ...
        "latest_verification_status": _verification_summary_for_current_context(verification_state).get(
            "status", "not_started"
        ),
    ...
    state["required_source_files"] = list(
        required_source_files if required_source_files is not None else source_files
    )
    ...
    state["latest_verification_status"] = _verification_summary_for_current_context(verification_state).get(
        "status", "not_started"
    )
...
def _write_living_doc(working_dir: str, filename: str, content: str, version: int, role: str) -> bool:
    """
    ...
    header = f"# {role} — Version v{version}\n# Last updated: {ts}\n{'=' * 60}\n\n"
    try:
        with open(path, "w", encoding="utf-8") as f:
...
def _source_output_relkeys(
    working_dir: str, output_files: list[str] | None, excluded_relpaths: list[str] | None = None
) -> set[str]:
    keys: set[str] = set()
...
def _source_workspace_name_matches(working_dir: str, filename: str) -> list[tuple[str, str]]:
    ...
    for dirpath, dirnames, filenames in os.walk(root):
        dirnames[:] = sorted(
            d for d in dirnames if d not in SOURCE_CONTEXT_PRUNE_DIRS and not d.startswith(".git")
        )
...
def _source_context_exclusion(
    path: str, relpath: str, stat: os.stat_result | None = None, *, direct_request: bool = False
) -> tuple[str | None, bool]:
    rel = relpath.replace("\\", "/")
    lower = rel.lower()
...
def _extract_user_mentioned_source_paths(
    user_query: str, working_dir: str, output_files: list[str] | None = None
) -> list[dict]:
...
def _caller_provided_source_items(
    working_dir: str, caller_source_files: list[str] | None = None
) -> list[dict]:
    seen: set[str] = set()
    entries: list[dict] = []
...
def _build_source_manifest(
    working_dir: str,
    excluded_relpaths: list[str] | None = None,
    max_files: int = SOURCE_MANIFEST_MAX_FILES,
    user_query: str | None = None,
    output_files: list[str] | None = None,
    caller_source_files: list[str] | None = None,
) -> list[dict]:
    source_extensions = {
        ".txt",
        ".md",
    ...
    def add_excluded(
        path: str, relpath: str, reason: str, sensitive: bool, stat: os.stat_result | None = None
    ) -> None:
        key = _relpath_key(relpath)
    ...
    for dirpath, dirnames, filenames in os.walk(root):
        ...
        for dirname in sorted(dirnames):
            ...
            if (
                dirname in SOURCE_CONTEXT_PRUNE_DIRS
                or key in excluded_norm
                or _matches_ignore_pattern(rel, gitignore_patterns)
            ):
                continue
        ...
        for filename in sorted(filenames):
            ...
            tier = SOURCE_CONTEXT_TIER_PROJECT if priority <= 2 else SOURCE_CONTEXT_TIER_OPTIONAL
            tier_name = "project_context" if tier == SOURCE_CONTEXT_TIER_PROJECT else "optional_discoverable"
    ...
    return (
        required_items
        + optional_items[:optional_limit]
        + excluded_items[:SOURCE_CONTEXT_EXCLUDED_SUMMARY_MAX]
    )
...
def _detect_source_files(working_dir: str, excluded_relpaths: list[str] | None = None) -> list[str]:
    ...
    excluded_norm = {os.path.normcase(item.replace("\\", "/")) for item in (excluded_relpaths or [])}
...
def _extract_preflight_requirements(
    user_query: str,
    output_files: list[str] | None = None,
    source_manifest: list[dict] | None = None,
    ...
    def add(req: dict) -> None:
        key = (
            req.get("type", ""),
            req.get("value") or req.get("name") or req.get("url") or req.get("label", ""),
        )
        if key in seen:
            return
        seen.add(key)
    ...
    if re.search(
        r"(?i)\b(?:requires?|needs?|need)\b.{0,40}\b(browser|chrome|playwright|screenshot|ui)\b", text
    ):
        add({"type": "browser", "value": "browser", "label": "browser tooling", "category": "environment"})

    if re.search(
        r"(?i)\b(?:requires?|needs?|need)\b.{0,50}\b(package registry|npm registry|pypi|network)\b", text
    ):
        add(
            {
                "type": "package_registry",
                "value": "package registry",
                "label": "package registry/network",
                "category": "environment",
            }
        )
...
def _probe_preflight_requirement(requirement: dict) -> dict:
    ...
    if req_type == "local_url":
        ...
        message = (
            ...
            if ok
            else f"required environment variable {name} is missing"
        )
    elif req_type == "tool":
        name = req.get("name") or req.get("value") or ""
...
def _run_preflight_requirements(requirements: list[dict], probe_func=None) -> dict:
...
def _collect_capability_inventory(
    working_dir: str, codex_bin: str | None = None, requirements: list[dict] | None = None
) -> dict:
    tools_to_check = [
        "python",
        "git",
    ...
    credentials = {
        name: "present" for name, value in os.environ.items() if value and _extract_credential_names(name)
    }
    hints = []
...
def _project_has_tests(working_dir: str, relpaths: list[str]) -> bool:
    if any("/tests/" in f"/{rel.lower()}/" or rel.lower().startswith("tests/") for rel in relpaths):
        return True
    if any(
        os.path.basename(rel).lower().startswith("test_") and rel.lower().endswith(".py") for rel in relpaths
    ):
        return True
...
def _verification_code_artifact_paths(
    output_files: list[str] | None, changed_files: list[str] | None
) -> list[str]:
    code_exts = {
        ".py",
        ".js",
...
def _verification_manifest_signals(working_dir: str, source_manifest: list[dict] | None = None) -> set[str]:
...
def _research_negation_scopes(text: str) -> list[tuple[int, int]]:
    ...
    for match in re.finditer(
        r"\b(?:do\s+not|don't|dont|without|no|not|avoid|exclude|skip)\b", text, flags=re.IGNORECASE
    ):
        start = match.start()
...
def _is_local_current_source_task(text: str) -> bool:
    ...
    if has_factual_news_request and (
        not has_local_news_maintenance_context or has_external_news_subject_context
    ):
        return False
    ...
    if has_research_source_context or (
        has_external_source_word_context and not has_local_source_word_context
    ):
        return False
...
def _research_signal_matches(
    query: str, output_files: list[str] | None, local_paths: list[str] | None = None
) -> tuple[bool, list[str]]:
    query_text = query or ""
...
def _classify_congress_task(
    ...
    if (
        exts & code_exts
        or manifest_signals
        or re.search(r"\b(code|script|implement|fix|bug|refactor|module|function|class|cli|build)\b", query_l)
    ):
        classes.add(TASK_CLASS_CODE_STATIC)
        signals.append("code_or_config_signal")
    if _project_has_tests(working_dir, manifest_rels + changed) or re.search(
        r"\b(test|pytest|unit test|integration test)\b", query_l
    ):
    ...
    if (
        exts & ui_exts
        or any(
            os.path.basename(rel).lower() in ("vite.config.js", "vite.config.ts", "next.config.js")
            for rel in all_paths
        )
        or re.search(
            r"\b(ui|browser|frontend|front-end|react|vue|svelte|canvas|screenshot|visual|dom|web app|playwright)\b",
            query_l,
        )
    ):
    ...
    if req_types & {
        ...
        r"\b(api|database|credential|oauth|token|endpoint|service|connector|server|redis|postgres|mysql|mcp)\b",
        query_l,
    ):
    ...
    if re.search(
        r"\b(security|secret|credential|password|token|auth|sandbox|subprocess|orchestrator|codex|agent|congress|lock|redact|approval gate|verification)\b",
        query_l,
    ):
    ...
    evidence_map = {
        TASK_CLASS_DOCUMENTATION_ONLY: [
            ...
            "requested_structure_or_headings",
            "inspector_review",
        ],
...
def _append_required_missing_verification_gates(
    ...
    *,
    ...
    has_static_command = _verification_has_command(
        checks,
        (
            "python_syntax",
            "node_check",
    ...
    has_test_command = _verification_has_command(
        checks,
        (
            "pytest",
            "npm_test",
    ...
    if TASK_CLASS_SERVICE_INTEGRATION in classes:
        has_preflight_evidence = any(check.id.endswith(":preflight_requirements") for check in checks)
        has_service_command = _verification_has_command(
            checks,
            (
                "pytest",
                "npm_test",
...
def _line_has_concrete_source_identifier(line: str) -> bool:
    ...
    if ("github" in lower or "repository" in lower or "repo" in lower) and re.search(
        ...
        return True
    if re.search(
        r"\b(?:official document|named official document)\s*[:=-]\s*\S.{8,}", text, flags=re.IGNORECASE
    ):
        return True
    if re.search(
        r"\b[A-Z][A-Za-z0-9 .-]+\s+(?:standard|specification|manual|guide|documentation)\s+(?:v?\d|\(\d{4}\))",
        text,
    ):
        return True
    return False
...
def _source_candidate_lines(text: str) -> list[str]:
    ...
    for raw_line in text.splitlines():
        ...
        if re.match(r"^#{1,6}\s+.+", line):
            in_sources = bool(
                re.search(r"\b(?:sources?|references?|bibliography|research evidence)\b", lower)
            )
            continue
        if re.match(
        ...
        if re.search(r"\b(?:sources?|references?|bibliography)\s*[:=-]", lower):
            ...
            lines.append(line)
            continue
        if in_sources or re.search(
            r"https?://|\bdoi\b|\barxiv\b|\bssrn\b|\bgithub\b|\brepository\b|\bofficial document\b", lower
        ):
            lines.append(line)
    return lines
...
def _research_evidence_from_text(text: str) -> dict:
    ...
    concrete_sources = [line for line in source_lines if _line_has_concrete_source_identifier(line)]
    vague_source_lines = [
        line
    ...
    field_checks = [
        ...
        (
            "not_checked_reasons",
            re.search(
                r"\b(?:not checked|not-checked|unchecked|blocked by|could not check|not used as evidence)\b",
                text,
                flags=re.IGNORECASE,
            ),
        ),
        (
            ...
            re.search(
                r"\b(?:stale|unverified|staleness|source warnings?|warnings?)\b", text, flags=re.IGNORECASE
            ),
        ),
    ]
...
def _latest_current_info_evidence_check(
    checks: list[VerificationCheck | dict], phase: str | None = None
) -> dict | None:
    matches = []
    for check in checks or []:
        item = _verification_check_to_dict(check)
        if item.get("id", "").endswith(":current_info_evidence") and (
            phase is None or item.get("phase") == phase
        ):
            matches.append(item)
...
def _select_verification_checks(
    ...
    checks.extend(
        _discover_verification_command_checks(
            working_dir,
            classification,
            phase,
            output_files=output_files,
...
def _run_non_command_verification_check(
    ...
    *,
    ...
    if check.id.endswith(":requested_outputs"):
        ...
        invalid = [
            item.get("path") for item in snapshots if item.get("invalid_type") or not item.get("exists")
        ]
        if check.phase == VERIFICATION_PHASE_BASELINE and invalid:
            ...
            check.reason += " Missing or invalid requested outputs: " + ", ".join(
        ...
        if found:
            ...
            check.status = (
                VERIFICATION_STATUS_BLOCKED if check.required else VERIFICATION_STATUS_NOT_APPLICABLE
            )
        ...
        if check.phase == VERIFICATION_PHASE_BASELINE and missing:
            check.status = VERIFICATION_STATUS_NOT_APPLICABLE
            check.reason += (
                " Research evidence is not required before Researcher creates or updates requested outputs."
            )
        elif missing:
            check.status = VERIFICATION_STATUS_FAILED
...
def _verification_summary_from_checks(
    ...
    *,
    ...
    required_failed = [
        item for item in relevant if item.get("required") and item.get("status") == VERIFICATION_STATUS_FAILED
    ]
    required_blocked = [
        item
        for item in relevant
        if item.get("required") and item.get("status") == VERIFICATION_STATUS_BLOCKED
    ]
    if verification_mode == "off":
        status = VERIFICATION_STATUS_WAIVED
        ...
        item in (VERIFICATION_STATUS_PASSED, VERIFICATION_STATUS_WAIVED, VERIFICATION_STATUS_NOT_APPLICABLE)
        for item in statuses
    ):
        status = (
            VERIFICATION_STATUS_PASSED
            if any(item == VERIFICATION_STATUS_PASSED for item in statuses)
            else statuses[0]
        )
    else:
    ...
    return {
        ...
        "not_applicable": sum(
            1 for item in relevant if item.get("status") == VERIFICATION_STATUS_NOT_APPLICABLE
        ),
...
def _render_verification_markdown(
    *,
    ...
    if current_summary.get("cumulative_status") and current_summary.get(
        "cumulative_status"
    ) != current_summary.get("status"):
        lines.append(
...
def _verification_blocked_state_from_summary(
    summary: dict, checks: list[VerificationCheck | dict]
) -> dict | None:
    ...
    if not blocked:
        blocked = next(
            (item for item in check_dicts if item.get("status") == VERIFICATION_STATUS_BLOCKED), None
        )
    if not blocked:
        return None
...
def _review_context_marker(
    user_updates: list[dict] | None, verification_summary: dict | None, round_num: int | None
) -> str:
...
def _review_unresolved_issue_lines(unresolved_text: str) -> list[str]:
    return [line for line in _review_quality_lines(unresolved_text) if not _review_line_is_no_issue(line)]
...
def _clean_issue_title(line: str) -> str:
    ...
    text = re.sub(
        r"\b(?:must|should)\s+be\s+(?:fixed|addressed|resolved)\b.*$", "", text, flags=re.IGNORECASE
    )
    text = re.sub(
        r"\b(?:needs|need)\s+(?:to\s+)?be\s+(?:fixed|addressed|resolved)\b.*$", "", text, flags=re.IGNORECASE
    )
...
def _issue_records_to_state(issues: list[IssueRecord | dict] | None) -> dict:
    ...
    open_material = [
        item
        for item in records
        if item.get("materiality") == "material" and item.get("status") not in ISSUE_CLOSED_STATUSES
    ]
    return {
        "issues": records,
...
def _extract_structured_blocks(section_text: str) -> list[dict]:
    ...
    for raw in (section_text or "").splitlines():
        ...
        loc_match = re.match(
            r"^(?:[-*]\s*)?(?:Location changed|Location|File|Path)\s*:\s*(.+)$", line, re.IGNORECASE
        )
        if loc_match:
            current["location"] = loc_match.group(1).strip()
        reason_match = re.match(
            r"^(?:[-*]\s*)?(?:Reason|Evidence|Rationale|Blocker)\s*:\s*(.+)$", line, re.IGNORECASE
        )
        if reason_match:
    ...
    for block in blocks:
        ...
        if "rationale" not in block:
            ...
            for line in block.get("lines") or []:
                if re.match(
                    r"^(?:[-*]\s*)?(?:Issue|Finding|Status|Location changed|Location|File|Path)\s*:",
                    line,
                    re.IGNORECASE,
                ):
                    continue
...
def _extract_excellence_checklist(
    ...
    *,
    ...
    missing = [key for key in EXCELLENCE_CHECKLIST_KEYS if key not in items]
    for key in missing:
...
def _issue_records_match(left: IssueRecord | dict, right: IssueRecord | dict) -> bool:
    ...
    same_severity_band = _issue_severity_band(ldata.get("severity")) == _issue_severity_band(
        rdata.get("severity")
    )
...
def _issue_lifecycle_blockers_from_records(issues: list[IssueRecord | dict]) -> list[str]:
    ...
    for raw in issues or []:
        ...
        if issue.status in ISSUE_OPEN_STATUSES:
            blockers.append(f"Unresolved material issue `{issue.id}` remains {issue.status}: {title}")
            ...
            if not _issue_status_has_valid_rationale(issue.status, issue.rationale, issue.title):
                blockers.append(f"Issue `{issue.id}` has invalid {issue.status} rationale: {title}")
    return blockers
...
def _review_issue_section_text(review_text: str) -> str:
    ...
    issue_heading_keys = {re.sub(r"[^a-z0-9]+", " ", alias.lower()).strip() for alias in issue_aliases}
    sections = [
        content.strip()
        for heading, content in headings.items()
...
def _review_line_is_issue_candidate(line: str) -> bool:
    ...
    if not cleaned or _review_line_is_no_issue(cleaned):
        return False
    if _review_line_has_concrete_non_material_rationale(
        cleaned
...
def _review_record_has_concrete_accepted_risk(record: dict, rationale_lines: list[str]) -> bool:
    ...
    for line in rationale_lines:
        if _review_line_has_concrete_accepted_risk_rationale(line) and _review_lines_share_quality_topic(
            issue_line, line
        ):
            return True
    return False
...
def _review_quality_signals(
    ...
    unresolved_for_quality = "\n".join(
        line
        for line in (unresolved_text or "").splitlines()
    ...
    material_records = [
        item for item in severity_records if item.get("severity") in QUALITY_MATERIAL_SEVERITIES
    ]
    ...
    actionable_unresolved = any(
        _review_text_has_any(line, QUALITY_ACTIONABLE_TERMS) for line in unresolved_lines
    )
    unresolved_blockers = [
        line
        for line in unresolved_lines
    ...
    if mode == QUALITY_MODE_BEST and raw_verdict == REVIEW_VERDICT_APPROVED:
        ...
        if material_blockers:
            for item in material_blockers[:5]:
                blockers.append(f"Material issue remains despite APPROVED verdict: {item.get('line')}")
            if len(material_blockers) > 5:
                blockers.append(f"{len(material_blockers) - 5} additional material severity issue(s) remain.")
        low_records = [item for item in severity_records if item.get("severity") == "LOW"]
        low_blockers = [
            item for item in low_records if not _review_record_has_concrete_low_rationale(item, review_lines)
        ]
    ...
    *,
    ...
    if expected_hashes and len(referenced_hashes) != len(expected_hashes):
        ...
        stale_reasons.append(
            "Review does not reference current requested-output sha256 hash(es): " + ", ".join(missing[:3])
        )
    ...
    if missing_sections:
        malformed_reasons.append(
            "Missing required Markdown review section(s): " + ", ".join(missing_sections)
        )
    ...
    unresolved_record = "\n".join(
        line
        for line in unresolved_content.splitlines()
        if not re.match(r"\s*VERDICT\s*:", line, re.IGNORECASE)
    ).strip()
    ...
    return ReviewAssessment(
        ...
        artifact_path=artifact_path,
        round=round_num,
        verdict=quality_signals.get("effective_verdict")
        if status
        in (
            REVIEW_STATUS_APPROVED,
            REVIEW_STATUS_NEEDS_REVISION,
...
def _is_only_nonfatal_codex_stderr(stderr: str, rc: int = 1) -> bool:
    ...
    if not (stderr or "").strip():
        return True
    if (
        _matches_context_limit_error(stderr)
        or _matches_rate_limit_error(stderr)
        or _is_network_error(stderr, 1)
    ):
        return False
    ...
    for line in error_lines.splitlines():
        ...
        rollout_bookkeeping = (
            "failed to record rollout items" in lower and "thread" in lower and "not found" in lower
        )
...
class Congress:
    ...
    def __init__(
        ...
        codex_bin_resolved: str | None = None,
        working_dir: str | None = None,
        verification_mode: str = DEFAULT_VERIFICATION_MODE,
        max_verification_timeout: int = DEFAULT_MAX_VERIFICATION_TIMEOUT,
        second_inspector_mode: str = DEFAULT_SECOND_INSPECTOR_MODE,
        ...
        from .policies import require_dedicated_worktree

        require_dedicated_worktree(self.working_dir)
    ...
    def _restore_resume_context_from_state(
        ...
        if not saved_state or not initial_substep:
            return
        if saved_state.get("original_request_hash") and saved_state.get(
            "original_request_hash"
        ) != _hash_text(user_query):
        ...
        if isinstance(review_state, dict) and review_state:
            assessments = [
                dict(item) for item in (review_state.get("assessments") or []) if isinstance(item, dict)
            ]
            if assessments:
        ...
        outputs_still_match_review = bool(latest_inspector_1) and not _assessment_matches_current_outputs(
            latest_inspector_1, current_outputs
        )
        if (
    ...
    def _write_partial_output_artifacts(
        self, agent: str, round_label, recovery_type: str, stdout: str = "", stderr: str = ""
    ) -> dict:
    ...
    def _save_retry_state(
        ...
        if retry_after_seconds is not None:
            retry_state["next_retry_at"] = (
                datetime.utcfromtimestamp(time.time() + int(retry_after_seconds))
    ...
    def _write_current_session_request(self, user_query: str, audience: str = "default") -> bool:
    ...
    def _record_user_update(
        ...
        self.logger.log_master(
            "SYSTEM",
            f"User update recorded source={source} redacted={str(update.get('redacted')).lower()}",
        )
    ...
    def _source_file_section(self) -> str:
        ...
        if not required_files:
            return ""
        file_list = "\n".join(f"  - {_relpath_from_workspace(self.working_dir, f)}" for f in required_files)
        return (
            f"\n{'=' * 60}\n"
    ...
    def _review_summary_for_state(self) -> dict:
        ...
        if self.final_approval_assessment is not None:
            ...
            self.review_summary["final_approval_status"] = (
                self.final_approval_assessment.status if self.final_approval_assessment.approved else None
            )
            if gate.get("waivers"):
    ...
    def _apply_issue_status_record(self, record: dict, *, source: str) -> None:
        ...
        if status == ISSUE_STATUS_PARTIALLY_ADDRESSED:
            ...
            if issue.materiality == "material" and status in (
                ISSUE_STATUS_ACCEPTED_RISK,
                ISSUE_STATUS_NON_MATERIAL,
            ):
    ...
    def _assess_review_output(
        self, reviewer: str, review_text: str, artifact_path: str, round_num: int
    ) -> ReviewAssessment:
    ...
    def _run_verification_phase(
        self, user_query: str, phase: str, changed_files: list[str] | None = None
    ) -> dict:
        ...
        for check in selected:
            ...
            if check.check_type == "command":
                ...
                executed.append(
                    _run_non_command_verification_check(
                        check,
                        working_dir=self.working_dir,
                        output_files=self.output_files,
                        source_manifest=self.source_manifest,
    ...
    def _ensure_final_verification_phase(
        ...
        if (self.verification_summary or {}).get(
            "latest_phase"
        ) == VERIFICATION_PHASE_FINAL and self._final_verification_output_signature == current_signature:
    ...
    def _verification_terminal_blocker(
        self, phase_summary: dict
        ...
        if status == VERIFICATION_STATUS_BLOCKED:
            ...
            if blocked:
                return (
                    ...
                    [blocked.get("reason", "Verification blocked.")],
                    blocked,
                )
            return (
                STATUS_BLOCKED_NEEDS_ENVIRONMENT,
                ["Verification is blocked by a required dependency."],
                None,
            )
    ...
    def _final_gate_should_continue(self, gate: FinalApprovalAssessment, round_num: int) -> bool:
        ...
        return any(any(term in str(reason) for term in retryable_terms) for reason in gate.blocking_reasons)
    ...
    def _build_final_approval_assessment(
        ...
        if self.blocked_state:
            ...
            status = (
                blocked_status if blocked_status in CONTROLLED_STATUSES else STATUS_BLOCKED_NEEDS_ENVIRONMENT
            )
            blockers.append(
        ...
        if verification_summary_for_gate.get("latest_phase") == VERIFICATION_PHASE_FINAL and isinstance(
            latest_phase_summary, dict
        ):
        ...
        verification_terminal, verification_reasons, verification_blocked = (
            self._verification_terminal_blocker(verification_summary_for_gate)
        )
        ...
        if not inspector_1:
            ...
            inspector_1.get("status") != REVIEW_STATUS_APPROVED
            or inspector_1.get("verdict") != REVIEW_VERDICT_APPROVED
        ):
            if status in APPROVED_STATUSES:
                status = STATUS_REVIEW_FAILED
            ...
            if (
                not inspector_1.get("session_marker_present")
                or inspector_1.get("session_marker") != current_marker
            ):
                if status in APPROVED_STATUSES:
                    status = STATUS_REVIEW_FAILED
        ...
        if second_inspector_required:
            if not inspector_2:
                ...
                inspector_2.get("status") != REVIEW_STATUS_APPROVED
                or inspector_2.get("verdict") != REVIEW_VERDICT_APPROVED
            ):
                if status in APPROVED_STATUSES:
                    status = STATUS_REVIEW_FAILED
                ...
                if (
                    not inspector_2.get("session_marker_present")
                    or inspector_2.get("session_marker") != current_marker
                ):
                    if status in APPROVED_STATUSES:
                        status = STATUS_REVIEW_FAILED
                    blockers.append(
                        "Inspector 2 review does not reference the current review freshness marker."
                    )
            ...
            waivers.append(waiver_reason or "Inspector 2 was explicitly waived by --second-inspector=off.")
        ...
        for review in latest_reviews_for_excellence:
            ...
            for item in checklist:
                ...
                if status_value in (EXCELLENCE_STATUS_FAIL, EXCELLENCE_STATUS_UNKNOWN):
                    ...
                    status_value == EXCELLENCE_STATUS_WAIVED and not str(record.get("evidence") or "").strip()
                ):
                    excellence_blockers.append(
        ...
        if issue_blockers and not any(
            item.get("key") == "material_issues_resolved" for item in excellence_records
        ):
            excellence_blockers.append(
        ...
        assessment = FinalApprovalAssessment(
            ...
            unresolved_material_issues=[
                _issue_record_to_dict(item)
                for item in self.issue_records
    ...
    def _build_researcher_prompt(self, user_query: str, round_num: int) -> str:
        ...
        living_doc_lines = []
        if has_res_doc:
            living_doc_lines.append(f"  Your previous notes/process report (latest): {res_doc_path}")
        if has_ins_doc:
            living_doc_lines.append(f"  Inspector feedback/comments (latest):      {ins_doc_path}")
        if has_ins2_doc:
            living_doc_lines.append(f"  Inspector 2 feedback/comments (latest):    {ins2_doc_path}")
        ...
        steps = []
        steps.append(
            f"Read the managed session request file: {os.path.join(self.working_dir, SESSION_REQUEST_FILE)}"
        )
        steps.append(
            "Re-read the ORIGINAL USER REQUEST included inline below. It remains the source of truth every round."
        )
        steps.append(
    ...
    def _build_inspector_prompt(self, user_query: str, researcher_output: str, round_num: int) -> str:
        original_request_section = self._original_request_prompt_section(user_query)
        source_section = self._source_file_section()
        optional_source_section = self._optional_source_context_prompt_section()
        output_section = self._output_file_section()
        ...
        living_docs_section = (
            f"\n{'=' * 60}\nLIVING DOCUMENTS — READ THESE COMPLETELY BEFORE STARTING:\n{'=' * 60}\n"
        )
        if has_res_doc:
            living_docs_section += f"  Researcher's latest explanation/process notes: {res_doc_path}\n"
        else:
            living_docs_section += (
                f"  [WARNING] {RESEARCHER_UPDATED_FILE} not found on disk. Using inline copy below.\n"
            )
        if has_ins_doc:
            living_docs_section += f"  Your previous review comments: {ins_doc_path}\n"
    ...
    def _build_inspector_2_prompt(
        self, user_query: str, researcher_output: str, inspector_1_output: str, round_num: int
    ) -> str:
        ...
        try:
            while True:
                ...
                if wait_count % 75 == 0:
                    ...
                    self.ui.status(f"Still paused ({elapsed:.0f}s)... [R] resume  [Q] quit", C_DIM)
        finally:
    ...
    def _wait_for_rate_limit(
        ...
        *,
        ...
        self.ui.status(
            f"Press [R] to retry now. [Q] to quit and save state. Auto-retry in {auto_minutes}m.", C_YELLOW
        )
            ...
            while True:
                if _kbhit():
                    ...
                    if ch == "r":
                        ...
                        self.logger.log_master(
                            "SYSTEM", f"Rate limit retry requested by user after {waited}s"
                        )
                        return "manual_retry"
                    elif ch == "q":
                        self._interrupt_requested = True
                        self.logger.log_master("SYSTEM", f"Rate limit quit requested after {waited}s")
                        return "quit"
                ...
                if now >= deadline:
                    ...
                    self.logger.log_master("SYSTEM", f"Rate limit auto-retry after {waited}s")
                    return "auto_retry"
                ...
                if waited >= 30 and status_bucket != last_status_bucket:
                    ...
                    self.ui.status(
                        f"Still waiting ({waited}s)... auto-retry in {remaining}s [R] retry  [Q] quit", C_DIM
                    )
        finally:
            _exit_cbreak(old_term)
    ...
    def _run_with_recovery(
        ...
        def timeout_is_recoverable(err: str, code: int) -> bool:
            ...
            return "did not start within" in lowered or "timed out: zero activity" in lowered
        ...
        while True:
            ...
            if _looks_like_complete_agent_output(agent, stdout) and _is_only_nonfatal_codex_stderr(
                stderr, rc
            ):
                if (stderr or "").strip():
                    self.logger.log_master(
            ...
            if rc == -3 or _is_network_error(stderr, rc) or timeout_is_recoverable(stderr, rc):
                reason = "timeout" if timeout_is_recoverable(stderr, rc) else "network"
                self.logger.log_master(
                    "SYSTEM", f"Recoverable {reason} error on attempt {attempt + 1}: {stderr[:100]}"
                )
                partial_event = None
                if stdout.strip():
                ...
                if stdout.strip() and current_sid:
                    ...
                    while cont_num < MAX_CONTINUATIONS:
                        ...
                        if cont_rc == -3 or _is_network_error(cont_err, cont_rc):
                            ...
                            self.logger.log_master(
                                "SYSTEM", f"Network error mid-continuation, saving {len(accumulated)} chars"
                            )
                            partial_event = self._record_partial_recovery(
                                agent,
                                round_label,
                        ...
                        if is_limited:
                            ...
                            self.logger.log_master(
                                "SYSTEM", f"Rate limit mid-continuation, saving {len(accumulated)} chars"
                            )
                            partial_event = self._record_partial_recovery(
                                agent,
                                round_label,
                                "rate_limit_mid_continuation",
                        ...
                        if not cont_out.strip():
                            self.ui.status(
                                f"Continuation produced no new output — accepting {len(accumulated)} chars.",
                                C_YELLOW,
                            )
                            break

                        self.ui.status(
                            f"Got {len(cont_out)} more chars (total: {len(accumulated)}), continuing...",
                            C_YELLOW,
                        )
                    # Exhausted MAX_CONTINUATIONS — mark partial; do not report clean success
                    self.ui.status(
                        f"Max continuations reached — saved {len(accumulated)} partial chars.", C_YELLOW
                    )
                    self.logger.log_master(
                        "SYSTEM",
                    ...
                    self.logger.log_master(
                        "SYSTEM", f"Context limit, no session ID, partial chars={len(stdout)}"
                    )
                    partial_event = self._record_partial_recovery(
                        agent,
                        round_label,
                        "context_limit_no_session",
                    ...
                    self.ui.error(
                        "Context window exceeded with no output. The prompt may be too large for the model."
                    )
                    self.logger.log_master(
                        "SYSTEM", f"Context limit hit, no output (rc={rc}): {stderr[:200]}"
                    )
                    return stdout, stderr, rc, total_duration, current_sid
            ...
            if is_limited:
                ...
                if stdout.strip():
                    ...
                    self.logger.log_master(
                        "SYSTEM", f"Rate limit with partial output ({len(stdout)} chars) — retrying"
                    )
                    partial_event = self._record_partial_recovery(
                        agent,
                        round_label,
    ...
    def _transition_menu(self, next_agent: str, round_num: int, user_query: str | None = None) -> str:
        ...
        print(
            f"  {C_GREEN}  [C]{C_RESET} Continue to {next_agent}  "
    ...
    def _menu_countdown(self) -> str:
        for remaining in range(AUTO_CONTINUE_SECS, 0, -1):
            sys.stdout.write(f"\r  {C_DIM}  Auto-continue in {remaining}s... (press C/P/M/O/Q){C_RESET}    ")
            sys.stdout.flush()
    ...
    def _add_interactive_comment(self, user_query: str | None = None, round_num: int = 1) -> None:
        try:
    ...
    def _menu_pause_loop(self, user_query: str | None = None, round_num: int = 1) -> str:
        ...
        try:
            while True:
                ...
                if wait_count % 75 == 0:
                    elapsed = wait_count * 0.2
                    self.ui.status(
                        f"Still paused ({elapsed:.0f}s)... [R] Resume  [M] Comment  [O] Output  [Q] Quit",
                        C_DIM,
                    )
        finally:
            _exit_cbreak(old_settings)
    ...
    def _build_researcher_fallback_notes(
        self, round_num: int, before_status: list[dict], after_status: list[dict]
    ) -> str:
    ...
    def _build_missing_output_repair_instruction(
        self, round_num: int, missing_outputs: list[str], invalid_outputs: list[str]
    ) -> str:
        lines = [
    ...
    def _build_no_output_review_fallback(
        self, reviewer_label: str, artifact_name: str, round_num: int, rc: int, stderr: str
    ) -> str:
        marker = self._review_context_marker(round_num)
        hashes = _current_output_hashes(self.working_dir, self.output_files)
        hash_lines = []
        for item in hashes:
            ...
            if item.get("invalid_type"):
                state = "invalid"
            hash_lines.append(f"- {item.get('path')}: {state}, sha256={item.get('sha256') or 'unavailable'}")
        reason = _redact_sensitive_text((stderr or "").strip()[:1000]) or "no stderr detail"
        title = (
            "Inspector 2 Review Unavailable"
            if reviewer_label == "Inspector 2"
            else "Inspector Review Unavailable"
        )
        return "\n".join(
            [
                f"# {title}",
    ...
    def _save_final_state(
        self, user_query: str, current_round: int, result: CongressResult, resume_substep: str | None = None
    ) -> None:
        current_substep = (
            resume_substep
    ...
    def _finalize_without_workspace_writes(
        self, user_query: str, status: str, blocking_reasons: list[str] | None = None
    ) -> str:
    ...
    def _finalize_result(
        ...
        if collisions:
            ...
            reasons.append(
                "Requested output collides with a Congress-managed artifact: " + ", ".join(collisions)
            )
        ...
        verification_data = (
            verification
            or _verification_summary_for_current_context(self.verification_summary)
            or _default_verification_placeholder(final_status)
        )
        ...
        if not collisions and not artifact_path_errors:
            try:
                ...
                artifact_error = f"Could not write Congress result/history artifacts: {type(e).__name__}: {e}"
            ...
            result_path_has_error = any(item.startswith(self.result_file) for item in artifact_path_errors)
    ...
    def _finalize_internal_error(
        self, user_query: str, exc: Exception, current_round: int | None = None
    ) -> str:
    ...
    def run(
        ...
        saved_user_updates: list[dict] | None = None,
    ) -> str:
    ...
    def _run_inspector_2_stage(
        ...
        self.logger.log_agent_output("inspector_2", round_num, stdout2, stderr2, rc2, duration2)
        if new_sid2:
        ...
        if rc2 == -2:
            ...
            final_output = self._finalize_result(
                user_query,
                STATUS_INTERRUPTED_RESUMABLE,
                blocking_reasons=["Inspector 2 was interrupted; resume will continue review for this round."],
                current_round=round_num,
        ...
        _write_round_output(self.working_dir, round_num, "inspector_2", inspector_2_output)
        _write_living_doc(
            self.working_dir, INSPECTOR_2_COMMENTS_FILE, inspector_2_output, round_num, "INSPECTOR 2 REVIEW"
        )
        round_data["inspector_2_output"] = inspector_2_output
        self.ui.status(f"Inspector 2 finished ({duration2:.0f}s, {len(inspector_2_output)} chars)", C_MAGENTA)
        inspector_2_assessment = self._record_review_assessment(
            self._assess_review_output(
                "inspector_2",
                inspector_2_output,
        ...
        if inspector_2_assessment.status in (REVIEW_STATUS_MALFORMED, REVIEW_STATUS_STALE):
            ...
            final_output = self._finalize_result(
                ...
                STATUS_REVIEW_FAILED,
                blocking_reasons=reasons
                or ["Inspector 2 review did not satisfy the Markdown review contract."],
                current_round=round_num,
            )
    ...
    def _run_core(
        ...
        if (
            saved_source_manifest
            and source_context_version == SOURCE_CONTEXT_VERSION
            and not self.source_manifest
        ):
            self.source_manifest = list(saved_source_manifest)
        self.required_source_files = [
            item["path"] for item in self.source_manifest if item.get("required") and item.get("path")
        ]
        ...
        if self.preflight_state.get("blocked"):
            ...
            self.logger.log_session_end(
                final_output, 0, blocked.get("status", STATUS_BLOCKED_NEEDS_ENVIRONMENT)
            )
            self.ui.final_result(0, self.logger.session_dir)
            return final_output
        ...
        if not self._write_current_session_request(user_query):
            ...
            self.ui.final_result(0, self.logger.session_dir)
            return final_output
        baseline_status, baseline_reasons, baseline_blocked = self._verification_terminal_blocker(
            baseline_summary
        )
        ...
        if self.required_source_files:
            ...
            self.ui.status(
                "No required source/input files detected from the user prompt or direct inputs.", C_DIM
            )
        ...
        if start_round > 1:
            self.ui.status(
                f"RESUMING from round {start_round} (substep: {initial_substep or 'start'})", C_GREEN
            )
        ...
        for round_num in range(start_round, self.max_rounds + 1):
            ...
            if skip_researcher:
                ...
                if loaded:
                    ...
                    round_data["output_status"] = _collect_output_status(self.working_dir, self.output_files)
                    self.ui.status(
                        f"[Resume] Round {round_num}: researcher notes loaded from disk "
                        f"({len(researcher_output)} chars).",
                        C_GREEN,
                    )
                    self.logger.log_master("SYSTEM", f"Resume: loaded researcher notes for round {round_num}")
                else:
                    self.ui.error(
                        f"Resume: expected round_{round_num}_researcher.md not found. Re-running researcher."
                    )
            ...
            if not skip_researcher:
                ...
                self.ui.status(
                    f"Researcher finished ({duration:.0f}s, {len(researcher_output)} chars of notes)", C_BLUE
                )
            ...
            if resume_at_inspector_2 and round_num == start_round:
                ...
                if inspector_output and researcher_output:
                    ...
                    invalid_or_missing = [
                        item["path"]
                        for item in final_output_status
                    ...
                    round_data["inspector_review_assessment"] = _review_assessment_to_dict(
            ...
            invalid_or_missing = [
                item["path"]
                for item in pre_inspector_output_status
                if item.get("invalid_type") or not item["exists"]
            ]
            if invalid_or_missing:
                self.ui.error(
            ...
            pre_inspector_final_summary = self._ensure_final_verification_phase(
                user_query,
                changed_files=[
                    item.get("path") for item in pre_inspector_output_status if item.get("exists")
                ],
            )
            final_status, final_reasons, final_blocked = self._verification_terminal_blocker(
            ...
            self.ui.agent_header("INSPECTOR", round_num, self.max_rounds, "Reviewing Deliverables")

            inspector_prompt = self._build_inspector_prompt(user_query, researcher_output, round_num)
            ...
            if not inspector_output:
                ...
                self.logger.log_master("SYSTEM", f"Inspector no-output fallback, rc={rc}: {stderr[:200]}")
                ...
                _write_living_doc(
                    self.working_dir, INSPECTOR_COMMENTS_FILE, inspector_output, round_num, "INSPECTOR REVIEW"
                )
            ...
            self.ui.status(f"Inspector finished ({duration:.0f}s, {len(inspector_output)} chars)", C_MAGENTA)
            ...
            if inspector_assessment.status in (REVIEW_STATUS_MALFORMED, REVIEW_STATUS_STALE):
                ...
                final_output = self._finalize_result(
                    ...
                    STATUS_REVIEW_FAILED,
                    blocking_reasons=reasons
                    or ["Inspector 1 review did not satisfy the Markdown review contract."],
                    current_round=round_num,
                )
                session_status = STATUS_REVIEW_FAILED
                break
            ...
            verdict = (
                inspector_assessment.effective_verdict
                or inspector_assessment.verdict
                or REVIEW_VERDICT_NEEDS_REVISION
            )
            ...
            if round_num == self.max_rounds:
                ...
                for latest in (
                    ...
                    if latest:
                        reviewer_label = (
                            "Inspector 2" if latest.get("reviewer") == "inspector_2" else "Inspector 1"
                        )
        ...
        if not final_output:
            final_output = self._finalize_result(
                ...
                session_status,
                blocking_reasons=[]
                if session_status in APPROVED_STATUSES
                else [f"Terminal status: {session_status}"],
            )
...
def interactive_mode(
    ...
    while True:
        try:
            ...
            raw_outputs = input(f"  {C_CYAN}{C_BOLD}> Output files (comma-separated):{C_RESET} ").strip()
            print()
...
def _display_final(ui: TerminalUI, final: str):
    print()
    if len(final) > 2000:
        ui.box("SESSION RESULT (truncated)", final[:2000] + "\n... (see logs for full output)", C_GREEN)
...
def main():
    ...
    for arg in _normalize_cli_args(sys.argv[1:]):
        if arg.startswith("--max-rounds="):
            ...
            print(
                "  Inspector 1 review, Inspector 2 review unless explicitly waived, and blocked-state absence."
            )
            ...
            print(
                "  approved_with_waiver means an explicit waiver, such as --second-inspector=off, was used."
            )
            sys.exit(0)
    ...
    if query:
        ...
        try:
            ...
            if final:
                print(final)
                exit_code = congress.last_result.exit_code if congress.last_result is not None else 1
                sys.exit(exit_code)
